...
void cRocket::update(float deltaTime)
{
	...
	{
		spriteRotation = 180.0f;
	}
	if (m_InputMgr->isKeyDown(VK_LEFT))
	{
		spriteTranslation = (glm::vec2(-3.0f, 0.0f));
		m_SoundMgr->getSnd("Tank")->playAudio(AL_TRUE);
	}
	...
	if (m_InputMgr->isKeyDown(VK_SPACE))
	{
		...
		theRocketBullets[numBullets]->setSpriteTranslation(glm::vec2(15.0f, 15.0f));
	...
	rocketVelocity *= 0.95;
	// set bounding rectagle for player
